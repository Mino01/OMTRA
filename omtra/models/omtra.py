...
class OMTRA(pl.LightningModule):
    ...
    def configure_loss_fns(self):
        ...
        # we need to disable time-scaled loss when there is no protein
        # unfortunately, this means that if time-scaled loss is enabled, and we have tasks that don't require protein, 
        # we need a second set of loss functions where reduction is always mean
        self._has_noprot_tasks = any([ 'protein_identity' not in task_name_to_class(task_name).groups_present for task_name in self.td_coupling.task_space ])
        self._alt_losses_enabled = self.time_scaled_loss and self._has_noprot_tasks
        self._alt_loss_dict = nn.ModuleDict() if self._alt_losses_enabled else None
        ...
        for modality in modalities_generated:
            if modality.is_categorical:
                ...
                if self._alt_losses_enabled:
                    self._alt_loss_dict[modality.name] = nn.CrossEntropyLoss(
                        weight=weights,
                        reduction='mean', 
                        ignore_index=-100
                    )
                ...
                if self._alt_losses_enabled:
                    self._alt_loss_dict[modality.name] = nn.MSELoss(reduction='mean')
    ...
    def forward(self, g: dgl.DGLHeteroGraph, task_name: str):
        ...
        # determine if we need to time-scale our losses
        time_scaled_loss = self.time_scaled_loss and ('protein_identity' in task_class.groups_present)
        ...
        for modality in task_class.modalities_generated:
            ...
            # dont compute loss if there are no nodes to compute it on
            if modality.is_node and g.num_nodes(modality.entity_name) == 0:
                losses[modality.name] = torch.tensor(0.0, device=g.device)
                continue

            if self._alt_losses_enabled and not time_scaled_loss:
                loss_fn = self._alt_loss_dict[modality.name]
            else:
                loss_fn = self.loss_fn_dict[modality.name]

            losses[modality.name] = (
                loss_fn(vf_output[modality.name], target)
                * weight
            )
            if self.time_scaled_loss:
