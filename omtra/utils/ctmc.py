...
def purity_sampling(
    ...
    if not m.is_node:
        ...
        etype = m.entity_name
        purities_ul = torch.zeros(g.num_edges(etype), device=device, dtype=purities.dtype)
        ...
        masked_nodes_ul = torch.zeros(g.num_edges(etype), device=device, dtype=torch.bool)
    ...
    with g.local_scope():
        if not m.is_node:
            data_src = g.edges
            topk_func = partial(dgl.topk_edges, etype=m.entity_name)
        else:
            data_src = g.nodes
            topk_func = partial(dgl.topk_nodes, ntype=m.entity_name)


        data_src[m.entity_name].data['purity'] = purities.unsqueeze(-1)
